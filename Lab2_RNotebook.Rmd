...
infNatSum<-read.csv(file.choose()) # Please choose nat_summary.csv from the data folder
A<-infNatSum$TOTAL.A
B<-infNatSum$TOTAL.B
...
infPosData<-read.csv(file.choose()) # Please choose Positive_tested.csv from the data folder
...
#influenza heat map
library(ggplot2)
infHeatData<-read.csv(file.choose())   # Please choose usa_state_map.csv from the data folder
...
infMData<-read.csv(file.choose()) # Please choose mortality.csv from the data folder
expe<-infMData$Expected
 thres<-infMData$Threshold
 percent<-infMData$Percent.of.Deaths.Due.to.Pneumonia.and.Influenza
plot(thres,ylim=c(4,12), xaxt="n",type = "l",xlab = "MMWR Week", ylab="% of all deaths due to P&I",col="black",font.axis=1)
axis(1, at=1:length(week), labels=infMData$Week,tck=0)
x_year_panel<- seq(1, length(week), by=40)
year_labels <- c("2009","2009",)
axis(1, at=x_year_panel,)
lines(expe,col="blue")
lines(percent,col="red")
legend("topright",fill=c("black","blue","red"),legend=c("Threshold","Expected","% of deaths due to P&I"),pch=1)
```

```{r}
#pediatric mortality data
ped_m_data<-read.csv(file.choose()) # Please choose pediatric_mortality.csv from the data folder
